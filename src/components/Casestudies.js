...
export default function CaseStudies() {
  return (
    <section id="case-studies" className="py-16 lg:py-24" style={{ background: "var(--beige)" }}>
      <div className="max-w-7xl mx-auto px-6">
        <p className="uppercase tracking-[3px] lg:tracking-[4px] text-sm text-center font-semibold" style={{ color: "var(--gold)" }}>
          Case Studies
        </p>
        <h2 className="font-serif text-3xl lg:text-5xl text-center mt-4 mb-6" style={{ color: "var(--green)" }}>
          Real engagements, real outcomes.
        </h2>
        <p className="max-w-3xl mx-auto text-center text-base lg:text-lg leading-7 lg:leading-8 mb-16" style={{ color: "#3A3A3A" }}>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          {caseStudies.map((study) => (
            <div
              ...
              style={{ background: "white", border: "1px solid rgba(13,59,46,0.12)" }}
            >
              <span
                className="text-xs uppercase tracking-widest font-semibold self-start px-3 py-1 rounded-full"
                style={{ background: "rgba(212,166,58,0.12)", color: "var(--gold)", border: "1px solid rgba(212,166,58,0.3)" }}
              >
                {study.tag}
              </span>
              ...
              <div>
                ...
                <p className="text-sm leading-6" style={{ color: "#3A3A3A" }}>{study.situation}</p>
              </div>

              <div>
                <p className="text-xs font-semibold uppercase tracking-wide mb-1" style={{ color: "var(--green)" }}>Action</p>
                <p className="text-sm leading-6" style={{ color: "#3A3A3A" }}>{study.action}</p>
              </div>

              <div className="pt-3 mt-auto" style={{ borderTop: "1px solid rgba(13,59,46,0.1)" }}>
                <p className="text-xs font-semibold uppercase tracking-wide mb-1" style={{ color: "var(--gold)" }}>Outcome</p>
                <p className="font-semibold text-sm leading-6" style={{ color: "var(--green)" }}>{study.outcome}</p>
              </div>
            </div>
          ))}
        </div>
